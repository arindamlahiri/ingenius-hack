...
def build_model(wordlen):
    model = Sequential()
    model.add(Embedding(wordlen, 300, input_length=500))
    for i in range(2):
    ...
    #model.add(Dense(256, activation='linear', activity_regularizer=regularizers.l2(0.01)))
    #model.add(keras.layers.Activation('relu'))
    model.add(Dense(2, activation='sigmoid', activity_regularizer=regularizers.l2(0.001)))
    #opt = keras.optimizers.Adam(beta_1=0.9, beta_2=0.999, epsilon=1e-07, amsgrad=False,name='Adam')
    #opt = keras.optimizers.SGD(learning_rate=0.00001, momentum=0.9, nesterov=False, name='SGD')
...
#importing dataset
dfan = pd.read_csv("annotations_metadata.csv")
dfan.head()

#reading labels from text file
x = []
for fn in dfan.file_id:
  f = open(fn+".txt", mode='r')
  x.append(f.read())
  f.close()
...
#extract one class
dfan2 = dfan1[dfan1.label=="noHate"]
dfan2.head()

dfan1=dfan

import random
randomList = random.sample(range(0, 10943), 2400)
dfan3=pd.DataFrame(data=None, columns = ['file_id','user_id','subforum_id','num_contexts','label','text'])

for i in randomList:
  if(i in dfan2.index):
    dfa2=pd.DataFrame(dfan2.loc[i])
    dfa2=dfa2.transpose(copy=True)
    dfan3=dfan3.append(dfa2)

#balancing data in both the classes
for p in range(5800):
  i = random.randint(0,10943)
  if(i in dfan1.index):
    if(dfan1.loc[i]['label']=='noHate'):
      dfan1=dfan1.drop(i)

for i in dfan1.index :
  if(dfan1.loc[i]['label']!='noHate' and dfan1.loc[i]['label']!='hate'):
    dfan1=dfan1.drop(i)

dfan1

len(dfan1[dfan1['label'] == 'noHate'])

len(dfan1[dfan1['label'] == 'hate'])

dfan1.to_csv('datafinal.csv')
